...
public class Generate {
    ...
    public static void main(String[] args){
        ...
        try{
            ...
            stmt.close();
            conn.close();
//            generatePojo(underline2Camel(DATA_BASE.replaceAll("tb_","").replaceAll("fe_","")),results);
...
//                "        DELETE FROM "+DATA_BASE+" t <include refid=\"keyWhere\" />\n" +
                "        DELETE FROM "+DATA_BASE+" WHERE id = #{id,jdbcType=VARCHAR}\n" +
                "    </update>\n"
                + "\n</mapper>";
        pojoContent+=tow;
        pojoContent+="\n}";
        ...
        //生成数据库实体类
        File pojo=new File("Areadme\\mysql\\"+className+"Pojo"+".java");
        if(pojo.exists()){       //如果存在这个文件就删除
            pojo.delete();
        }
        System.out.println(pojo.createNewFile());
        writeFileContent("Areadme\\mysql\\"+className+"Pojo"+".java", pojoContent);
        //生成mapper接口
        File dao=new File("Areadme\\mysql\\"+className+"Dao"+".java");
        if(dao.exists()){       //如果存在这个文件就删除
            dao.delete();
        }
        System.out.println(dao.createNewFile());
        writeFileContent("Areadme\\mysql\\"+className+"Dao"+".java", daoContent);
        //生成mapper数据库xml
        File mapper=new File("Areadme\\mysql\\"+className+"Mapper"+".xml");
        if(mapper.exists()){       //如果存在这个文件就删除
            mapper.delete();
        }
        System.out.println(mapper.createNewFile());
        writeFileContent("Areadme\\mysql\\"+className+"Mapper"+".xml", mapperContent);
    }
